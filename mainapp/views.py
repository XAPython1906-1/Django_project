...
def login(request):
    ...
    if request.method == 'POST':
        ...
        login_user = User.objects.filter(username=username, password=password, status=1).first()
        if login_user:
            # 系统管理员
            login_info = {
            ...
            login_user = User.objects.filter(username=username, password=password, status=2).first()
            if login_user:
                # 会员
                login_info = {
                    '_id': login_user.user_id,
